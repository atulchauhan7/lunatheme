...
/* ── GoKwik (KwikCheckout) ────────────────────────── */
if(window.theme&&window.theme.checkoutProvider==='gokwik'){
var gkMethods=['initCheckout','openCheckout','checkout'];
var gkApi=window.gokwik||window.GoKwik||window.GKwik;
if(gkApi){
for(var gi=0;gi<gkMethods.length;gi++){
if(typeof gkApi[gkMethods[gi]]==='function'){
try{gkApi[gkMethods[gi]]();return;}catch(e){}
}
}
}
...
h+='<form action="/cart" method="post" novalidate class="cart-drawer__checkout-form"><button type="button" id="checkout2" name="checkout2" class="btn btn--primary btn--full gokwik-checkout-btn" onclick="handleFloCheckoutBtn()">Checkout</button></form>';
